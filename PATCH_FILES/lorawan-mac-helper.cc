...
std::vector<int>
LorawanMacHelper::SetSpreadingFactorsDistributed(NodeContainer endDevices, const std::vector<uint8_t>& drValues)
{
    std::vector<int> sfQuantity(7, 0);
    uint32_t numDevices = endDevices.GetN();
    uint32_t numDrValues = drValues.size();

    NS_ASSERT_MSG(numDrValues > 0, "The array of data rates must not be empty");

    uint32_t devicesPerDr = numDevices / numDrValues;
    uint32_t remainingDevices = numDevices % numDrValues;

    auto drIt = drValues.begin();
    uint32_t currentDrCount = 0;
    ...
    for (auto j = endDevices.Begin(); j != endDevices.End(); ++j)
    {
        ...
        mac->SetDataRate(*drIt);
        sfQuantity[*drIt]++;

        currentDrCount++;
        if (currentDrCount >= devicesPerDr + (remainingDevices > 0 ? 1 : 0))
        {
            currentDrCount = 0;
