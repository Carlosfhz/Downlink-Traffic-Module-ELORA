...
LoraPacketTracker::LogUplinks(Time startTime, Time stopTime,NodeContainer gateways,NodeContainer endDevices, std::string filename){
    ...
    if (Simulator::Now() == Seconds(0))
    {
        ...
        NS_LOG_DEBUG("Deletes Content");

        outputFile.open(c, std::ofstream::out | std::ofstream::trunc);
    }
    else
    {
        NS_LOG_DEBUG("Append To file");
    ...
    NS_LOG_DEBUG("Printing Packets");

    outputFile << "gatewayID,X,Y,Z" << std::endl;

    for (auto it = gateways.Begin(); it != gateways.End(); ++it)
    {
        int systemId = (*it)->GetId();
        auto position = (*it)->GetObject<MobilityModel>()->GetPosition();
        outputFile << std::to_string(systemId) << ","
                   << position.x << ","
                   << position.y << ","
                   << position.z << std::endl;
    }
    NS_LOG_DEBUG("End device location");
    ...
    outputFile << "senderId,receiverID, sendTime, receivedTime,SF,SNR,Ftype, Freq,received, code"<< std::endl;

    NS_LOG_DEBUG("Constructign Uplink logs");
 ...
 */
            ...
            NS_LOG_DEBUG("Dealing with packet 2.5" << ppd.second.packet);

            NS_LOG_DEBUG("Dealing with packet 2" << ppd.second.packet);
            LoraTag tag;

            Ptr<Packet> copy = ppd.second.packet->Copy();
            ppd.second.packet->Copy()->RemovePacketTag(tag); // Needed in case of retx!
            for (const auto& out : ppd.second.pktInfoCom){
                NS_LOG_DEBUG("Writing " << ppd.second.senderId);
    ...
    outputFile << "senderId,receiverID, sendTime, receivedTime,SF,SNR,Ftype, Freq,RX"<< std::endl;

    NS_LOG_DEBUG("Constructing DL logs");
